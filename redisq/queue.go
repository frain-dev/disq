...
var _ disq.Queue = (*Queue)(nil)

func NewQueue(opt *disq.QueueOptions) *Queue {
	const redisPrefix = "disq:"
...
func (q *Queue) Options() *disq.QueueOptions {
	return q.opt
}
...
func (q *Queue) Add(msg *disq.Message) error {
	return q.add(q.redis, msg)
}

// Add messages to the stream, if delay > 0, add to the zset
func (q *Queue) add(pipe RedisStreamClient, msg *disq.Message) error {
	if msg.ID == "" {
		u := uuid.New()
...
func (q *Queue) Release(msg *disq.Message) error {
...
func (q *Queue) Delete(msg *disq.Message) error {
...
func (q *Queue) cleanZombieConsumers(ctx context.Context) (int, error) {
	...
	for _, consumer := range consumers {
		if time.Duration(consumer.Idle)*time.Millisecond > q.opt.WorkerIdleTimeout {
